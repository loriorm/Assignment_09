...
# ormerodl, 2020-Sept-09, Updated for todos
# ormerodl, 2020-Sept-09, add track properties
# ormerodl, 2020-Sept-09, add track methods code
# ormerodl, 2020-Sept-09, resolve syntax errors
# ormerodl, 2020-Sept-09, test functionality 
#------------------------------------------#
...
class Track():
    ...
    def position(self, value):
        if type(value) == int:
            if value < 1:
                raise Exception('Position cannot be be less than 1.')
...
class CD:
    ...
    def __sort_tracks(self):
        """ Sorts tracks using Track.position, any blanks equal none"""
        n = len(self.__tracks)
    ...
    def get_tracks(self) -> str:
        """Returns a string list of tracks on an album

        Raises:
            Exception: if no tracks on album
        Returns:
        ...
        if len(self.__tracks) < 1:
            raise Exception('No tracks on album')
        result = ''
        for track in self.__tracks:
            if track is None:
                result += 'No track info\n'
            else:
                result += str(track) + '\n'
        return result

    def get_long_record(self) -> str:
        """gets long version album info and track details

        Returns:
            result (string): Formatted info about album/tracks.
            """
